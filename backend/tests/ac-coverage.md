...
| AC-002 | `safety-and-query.test.js`：失败/超时回执禁止成功陈述；无回执聊天/知识使用固定模板并屏蔽多种同义成功文本 | 覆盖 |
...
| AC-060–AC-067 | `tasks-and-session.test.js`：唯一任务、await 后并发复核、按保存规格恢复、真实重复停止幂等 | 覆盖 |
